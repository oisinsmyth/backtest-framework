...
**[D580](../decisions/D580-STAGE-0-DESIGN-the-funding-clock-on-CME-bitcoin-premise-check.md) — Stage 0 DESIGN, committed, unrun: the funding-cycle premise on CME bitcoin.** T1 concentration of BTC one-minute volume and r² in [S−30, S+30) against the construction's own placement null (96 five-minute offsets through the 8-hour cycle; the seven other tops of the hour inside it; terminal if it fails); T2 the funding sign orders the 30-minute post-move (defaults excluded; placement and event-shift nulls); T3 magnitude; T4 the level path and era split with a bar 2023 must clear alone; T5 per settlement time with the 08:00 UTC winter London-open confound flagged. **Needs first:** `scripts/build_fut_btc_1m.py` → `fut_btc_1m.csv.gz` (BTC and MBT, every session, UTC-keyed, front by daily volume via D462's chunk function; gates G1–G5 in the design; decode on the system interpreter, projected 10–20 min). Reads only bars < 2024-01-01. The cost gate on the 2025-26 `tbbo` quotes at the funding timestamps is a read inside the reserved period and goes to the principal before it happens. Decision rule SUPPORTED / PARTIAL / NOT SUPPORTED in the record.
