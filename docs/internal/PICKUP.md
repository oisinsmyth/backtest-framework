...
**Expiry pinning on ES is measured and the line closes** ([D613 fixture](../decisions/D613-FIXTURE-the-ES-option-volume-panel-in-ET-clock-buckets.md), [D614 pre-registration](../decisions/D614-PRE-REG-expiry-pinning-on-ES-from-the-noon-0DTE-volume-ladder.md), [D614 result](../decisions/D614-STAGE-0-RESULT-not-pinning-ten-sessions-carry-the-whole-pull.md), 2026-09-22). The first post-deposit candidate. The noon 0DTE volume centroid's signed distance from the 15:30 price carries **−5.08 bp per sigma** on the last half hour at Newey-West t −2.11, and the pre-registered bars return **REAL BUT NOT TRADEABLE** on the economic bar, so nothing goes forward and the ES 2024+ slice stays unread. The reading beside that verdict is that it is **not pinning**: the sign is repulsion where the mechanism predicts attraction, **76.6 % of the coefficient comes from ten of 1,132 sessions**, White's HC3 gives −1.96 and Newey-West lag 10 −1.84, it is −5.73 before 2022-05 and −0.16 after, it rises with distance instead of falling, the two sensible strike bands are inside their nulls, and the grid-only placebo with no option content clears the rank bar at 0.987. A one-MES book loses in both directions. **Built and left behind:** `fut_es_0dte_volume_cutoffs.csv.gz` (5.3M rows, every traded expiry in five ET clock buckets, the 15:30 sum reproducing D581's committed column exactly), `scripts/stage0_d614_expiry_pin.py` with its estimator ladder, tail diagnostics, singularity guard and norm-preserving null, and four method facts in FINDINGS §79 — chief among them that `c_only` in D581's runner returns the interaction rather than the level, which any future level claim reusing it would get wrong. **Outstanding:** superseded on 2026-09-22 by the sharpened programme below — the aggressor-flagged year has now been read, as a census.

**The two data layers the sharpened ladder needs are built** ([D616](../decisions/D616-FIXTURE-the-open-interest-reference-session.md), [D617](../decisions/D617-FIXTURE-the-ES-option-signed-flow-census-from-the-tbbo-year.md), 2026-09-22). The principal asked for all six sharpenings D614 §7 names, staged as five measured in sample plus the aggressor improvement **as a fixture only**. **D616** put `oi_ref_session` on the option panel from the `ts_ref` already sitting in the cached statistics — a `--build` from cache, 42.3 min, no pull — so a difference of open interest can be *shown* to span one session instead of assumed to. `oi_pub_et` structurally cannot do that job: a publication is usable on exactly one session, so adjacent publication times are **never** equal (0.0000), which made CME's preliminary-then-final revision invisible. The publication kept is unchanged, and that is proven rather than claimed — the amended file with its last field stripped hashes to the pre-amendment file over all 19,225,750 lines, so **`oi` did not move and D581/D614 reproduce**. The gap is one session on 97.7–99.8 % of rows and **the residue grows with the era**: cells carrying two reference dates run 0.1875 in 2016 and **0.3945 in 2025**, spurious zero deltas 0.24 % and **2.53 %** — the reserved year is the worst year for any Δ-OI construction, and a differencing study must drop those pairs and count them. **D617** read the aggressor side on ES options for the first time (D511 had declined it): 1,440,501,590 trades from 13 `tbbo` files in 6.5 min at 97 % of linear → 1,044,886 rows over 124,580 options and 314 ET dates 2025-09-10 → 2026-09-10. The three numbers a later signed design needs: unsigned share **0.000166**, quotes usable 0.9908, and D485's at-quote agreement **0.999983** on 22,496,746 trades — 389 disagreements, which is what an independent check looks like, where exactly 1.0 would have suggested the book snapshot was derived from the trade and the check was worthless. **It scores nothing**: the header guard refuses any column that is not a contract or a trade count, and the window stays **unspent for every return-bearing construction** in D510/D511's shape. Two traps found by building rather than by designing: the ET calendar date puts the opening Globex evening one date *before* the pull window (now a declared one-evening allowance, asserted from both sides), and the panel in this worktree was a **hardlink** to the shared worktree's copy, so the rebuild would have written into another session's file through the shared inode. **Outstanding:** nothing — the result is below.

**The sharpened ladder is measured and the axis closes** ([D618 result](../decisions/D618-STAGE-0-RESULT-the-band-around-the-price-was-the-signal.md), 2026-09-22, amended the same day, runner `scripts/stage0_d618_sharpened_ladder.py`, 60 s). **Of 72 cells five pass the construction screens — including `gamma_doi|all|w30|now`, the pre-registered priority cell #1 — and none clears its own nulls.** The primary reads **+1.663, the PREDICTED attraction sign** where D614 read −5.08, at NW t **+0.96**, rank 0.757 in its own shift null and 0.508 in the flip null, expected move **$0.66 against a $4.25 round trip**, tail bar held. Family max |t| **1.961** against a family-max null p95 **2.714** with per-cell bars 1.86–2.13. Component line on the primary: net Sharpe −1.218/−1.301, net Sortino −1.705/−1.614, −$4.11/−$4.39 a session, ρ +0.034 with the MACD arm; gross is indistinguishable from zero, so cost is not what killed it.

**READ THIS BEFORE TRUSTING ANY SCREEN IN THIS REPOSITORY.** The first version of this record said "not one of 72 passes" and **it was wrong, because screen S1b was invalid**. The principal asked whether the screens were too harsh; a known-answer case answered it — S1b **rejected a conditioner built from the actual 16:00 close**, perfect information, at a ratio of 0.05, along with all 72 real cells. It compared each cell's dispersion against flat weights over its positive-weight strikes, which for any smooth weight is the whole 220-strike ladder whose centroid sits ~23σ away, so it penalised the very concentration a signal has. A permutation comparator inverts identically (oracle 0.04, grid 1.00). Replaced by `|corr(LADDER, PING)| <= 0.50`, calibrated on the controls alone (grid 1.0000 by construction, oracle/diluted/weak 0.0605/0.0359/0.0196). **A gate that rejects rather than reports must be shown to admit an oracle before its rejections are believed.**

**The finding that survived the correction, on statistics the broken screen never touched: the interior solution appeared and it was the circularity.** The runner's first pass centred each band on **P1530** — the pre-registered *break*. Re-anchoring on the **prior settlement** takes those cells from sd 0.62–1.04σ to **5.57–5.59σ**, corr(DAY0) from −0.19/−0.21 to **−0.54**, and corr with their own grid centroid from **+0.31 to +1.000**; passing cells go from **15 to 5**. The two bands pick different strikes on **90.7 %** of sessions (median Jaccard 0.333). **A band is a selection, and one centred on the outcome's own starting price is D614's grid artefact one level up.** The axis now closes on two independent legs: every pre-session-anchored weight carries corr(DAY0) −0.48 to −0.54 (−0.73 to −0.74 against the full 09:30→15:30 move), every banded cell is its own grid centroid at 0.998–1.000, and the one construction escaping both — Δ-OI unbanded, 13.1σ wide and −0.134 independent — scores **−0.0006 at t −0.01**. `WEIGHTS BITE` fired on the gamma-weighted cell as the record predicted (1.355 points against a 2.0 floor). **Ten corrections are listed in the record.** **Outstanding:** nothing on this axis; a future attempt must name where its dispersion comes from and show it is none of the three. Per-strike dealer inventory is not on this disk. FINDINGS §80.

**The tail of that study — the F5 continuation and its log-MACD filter — is CLOSED by the principal, 2026-09-22, on a power calculation** (record §3c–§3e). Identifying what the one gross-clearing cell was eating led to the plain 14:30→15:30 continuation (real: enumerated rotation rank 0.9990, survives symmetric trimming, but 2016–17 negative and the fee is 1.25× the gross at MES while one ES breaches the barrier 4.8×), and then to a log-MACD filter on it. The filtered cell was the **first construction in this programme with the right shape for a component**: MES net **+$3.09** a trade, Sharpe **≈0.46** by its own trade count, drawdown **0.29×** the allowance, ρ **−0.094** with the admitted arm, clearing a **best-of-five enumerated rotation null** (rank 0.9974 against p95 $4.20) with the signal side rotated jointly so the filter selection is priced. **It is closed anyway, and the reason generalises: at 42 trades a year, a two-year holdout returns an expected t of +0.65 EVEN IF THE EFFECT IS ENTIRELY REAL, +0.38 at the recent firing rate, and t = 2.0 would need 786 trades — 19 years.** Spending the unread 2024+ slice would buy a number near zero either way and destroy the holdout. Size does not help: at one ES the cell earns $57.87 a trade instead of $3.09 and **t is identical**, because scaling multiplies the mean and its standard error alike. Stripping the volatility leg does not rescue it either — 697 trades improve the hit rate to 0.547 with a positive median and zero tail dependence, but MES net falls to **+$0.70 at t +0.38** (a fixed $4.25 fee paid twice as often for half the gross: $488 total against the two-leg's $1,041), drawdown worsens to 5.68× at ES, and 2022–23 turns negative. **Reopening needs a construction firing 150+ times a year, or a mechanism argument that carries the prior without the statistics.** The ES day-session 2024+ slice stays unread.

**The sharpened ladder is pre-registered, and the stress test changed what it asks** ([D618](../decisions/D618-PRE-REG-the-sharpened-0DTE-ladder-range-or-independence.md), 2026-09-22, committed before the runner exists). A stress test measured the six sharpened objects in sample before anything was pre-registered, and the finding generalises past this line: **dispersion in "signed distance from the price to a weighted strike" comes either from the grid's relation to the current price — no information, sd 0.14σ — or from the day's own move, corr −0.55. There is no interior solution.** Gamma weighting does kill D614's worst confound (corr with DAY0 +0.002 against −0.515) but by collapsing the range 16×; anchoring gamma pre-session restores the range (sd 3.04σ) and *is* the day's return with a sign flip (−0.546, worse than the −0.489 D614 died of). D614's own volume centroid sits at the same point (3.07σ, −0.49), which retro-explains every one of its symptoms — largest where furthest, crisis afternoons, dead after 2022-05, repulsion. So the primary question is no longer whether the conditioner orders the close but **whether any of the six changes yields a conditioner with both range and independence**, and prediction 1 is the falsifiable form: no cell with sd ≥ 1.0σ and |corr(DAY0)| ≤ 0.40. All six are still built and measured. **Four pre-outcome screens** (a measurement floor at 0.20σ set deliberately *below* the 0.238σ measured for `|gamma|x|Δoi|` so improvements 1 and 2 are scored rather than screened out on a threshold chosen after seeing them; a placebo floor at 3× the flat-weight version; the D614 §4 confound ceiling; and grid-R² and strike-count floors that the ±1–2 step bands are predicted to fail), a **64-cell family** with a family-maximum null, a named **Frisch–Waugh gate** on Δ-OI residualised against both the OI and volume centroids (predicted corr +0.66 and +0.33 — if the coefficient lives in the shared part the record says "volume in disguise" and stops), and the **economic bar restated in dollars per trade at one MES** because D614's was stated in units of a dispersion assumed at 0.3 and measured at 3.07. **One error caught in the draft before committing it:** the screen's floor was first justified through a bp-to-dollars chain that does not hold, and redoing the arithmetic honestly showed that at D614's measured economics (+$2.49 gross a session at 3.07σ, needing 1.71×) clearing cost would take *more* dispersion than the closed construction had — so no dispersion floor can substitute for an economic bar at all, and the two bars are now separate by design. Honest prior stated in the record: **~4 %**.
