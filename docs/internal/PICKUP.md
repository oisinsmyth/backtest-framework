...
**Still on the deposit's list:** the combined continuous trend + carry forecast (§8) at equal weights was cell C of D556 (+0.20, drawdown −29%) — and now has no forward slice; a seasonal-adjusted carry for NG and the grains is a declared construction away, likewise without one; a name-randomised null (keep the leg counts, randomise who fills them) would separate tilt from timing and none of D557–D562 pre-registered one — D561 and D562 show why it matters (the rotation null's median is +0.11 / +0.18 on the Sharpe and −0.68 / −0.55 σ on ES's worst days, which is tilt); D555's long-window null is enumerated (D561 S1), the other four are not; the overlay on the ledger's live arm needs that arm's daily P&L on disk. **The three cross-sectional sorts (D557–D559) did not read 2024+ and keep it.**
