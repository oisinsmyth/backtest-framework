...
**Still on the deposit's list:** the combined continuous trend + carry forecast (§8) at equal weights was cell C of D556 (+0.20, drawdown −29%); a seasonal-adjusted carry for NG and the grains is a declared construction away; a name-randomised null (keep the leg counts, randomise who fills them) would separate tilt from timing and none of D557–D559 pre-registered one — D561 shows why it matters (the rotation null's median on ES's worst days is −0.68 σ, which is tilt); D555's long-window null is now enumerated (D561 S1), the other four are not; the overlay on the ledger's live arm needs that arm's daily P&L on disk.
