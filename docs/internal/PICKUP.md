...
**The sharpened ladder is measured and the axis closes** ([D618 result](../decisions/D618-STAGE-0-RESULT-the-band-around-the-price-was-the-signal.md), 2026-09-22, runner `scripts/stage0_d618_sharpened_ladder.py`, 53 s). **Of 72 cells not one passes the four construction screens, so no return was scored for any cell in the specified family.** The finding that matters more than the verdict: **the interior solution appeared and it was the circularity.** The runner's first pass centred each band on **P1530** — the pre-registered *break*, not the specification — and four cells passed every screen with one clearing both its own nulls (shift rank 0.966, flip 0.974). Re-anchoring the band on the **prior settlement** takes those same cells from sd 0.62–1.04σ to **5.57–5.59σ**, corr(DAY0) from −0.19/−0.21 to **−0.54**, and their flat-weight ratio from **3.05–3.54× to 1.00×**: selecting the strike set around the current price manufactured both the independence and the apparent weight information, and the two bands disagree on **90.7 %** of sessions (median Jaccard 0.333). **A band is a selection, and one centred on the outcome's own starting price is D614's grid artefact one level up.** Three sources of variance, not the two a review had argued: the grid's relation to the price, the day's move, and — new — **the listed ladder's extent**, which is why the Δ-OI centroid is wide (sd 13.1σ/19.0σ) *and* independent (−0.134/−0.142) *and* worthless, its flat-weight ratio being 1.52. An absolute dispersion floor would have passed a thirteen-sigma cell as signal; only the placebo-relative floor catches it. The artefact cells, scored with no verdict weight, fail anyway: family max |t| **2.336** against a family-max null p95 **2.608** while each cell's own p95 is 1.93–2.04 (four correlated cells cost ~0.57 of t), best expected move **$1.90 against a $4.25 round trip**, every coefficient **negative** (repulsion, as in D614), component net Sharpe −1.234/−1.166 and net Sortino −1.714/−1.414 at −$4.37/−$4.13 a session, ρ +0.055 with the MACD arm. `WEIGHTS BITE` **fired on the gamma-weighted cell exactly as the record predicted** (median |K_w − P1530| 1.355 points against a 2.0 floor). **Nine corrections are listed in the record**, four of which changed a published number and one the verdict; the second worth remembering is that comparing a **signed** observed coefficient against an **absolute** null makes every rank 0.000 by construction. **Outstanding:** nothing on this axis — it closes on a construction argument, and a future attempt must name which of the three variance sources its dispersion comes from and show it is none of them. Per-strike dealer inventory is not on this disk. FINDINGS §80.
