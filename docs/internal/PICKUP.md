...
**[D581](../decisions/D581-STAGE-0-DESIGN-gamma-conditioned-close-on-ES-the-discriminator.md) — Stage 0 DESIGN, committed, unrun: the gamma-conditioned close on ES, the deposit's §4 discriminator as a premise check.** T1: `R2 = a + b·F5 + c·F5·F2 + d·F2` with R2 the 15:30→16:00 return, F5 the 14:30→15:30 move, F2 the sign of carried dealer net gamma from prior-close option OI under the stated convention (calls +, puts −); predicted c < 0, NW t ≥ 2, rank ≥ 0.95 in the enumerated day-shift null of F2; kill 3 (wrong sign, never flipped), kill 4 as T3 (must be stronger 2022–2023 than 2016–2021 and hold in 2023 alone), T4 0DTE days vs the rest; the 0DTE volume build from the option minute bars ON DISK as a diagnostic (F1b, unsigned). **Blocked on a data decision:** option OI and definitions are not on disk; quoted 2026-09-20 (`scripts/quote_es_options_pull.py`, `data/es_options_pull_quote.json`) — `ES.OPT` statistics 37.41 GB + definition 10.28 GB, 2016-01 → 2026-09-11, **USD 0.00 under the subscription that ends ~2026-10-11**. On the principal's word: pull, then `build_fut_es_options_eod.py` (gates G1–G6 in the design; OI keyed on the session it is first usable, D497/D521), then the runner. Reads nothing from 2024-01-01 on.

**Still on the deposit's list:** the five published strategies are all scored and two lines are closed; basis-momentum is scored (D564, above); the NG avatar is scored and removed (D565, D566); the grains' Stage 0 is run (D567) and the corn post-harvest carry is pre-registered, run and does not pass (D568: the always-on spread beats the window per day; the merchant avatar has failed on corn in both halves of its year); the soybean harvest spread was pre-registered as seen on the F/H pair and does not pass (D570: third of its own twelve placements; the July → September X/F placement at +1.24 is a null cell, seen, Stage 0 material); **the grains have no open pre-registration** — the cross-root placement test (D571) failed at chance level, so the seen cells (corn U/Z Jun–Aug 13 of 13, soybeans X/F Jul–Sep, meal V/Z Sep, the corn spring narrowing of D568) are each pre-registrable only as seen with two forward windows; wheat's profile is flat; oil's is the bean's inverted; **the livestock Stage 0 (D575) refuted the hog supply-calendar avatar and supported neither cattle avatar — the seasonal-spread line has no open construction and its closure is the principal's call;** the avatar programme's remaining roots are livestock, then HO and CL; the grains at harvest (WASDE stocks-to-use, on disk) and livestock (disaggregated COT, on disk), then HO and CL, each its own pre-registration; what the deposit itself names and parks (`PUBLISHED_STRATEGIES.md` §0) is **hedging pressure** (Basu–Miffre; the programme's Phase 2 in `ALPHA_PROGRAMME.md` §4; the legacy COT fixture is on disk — `cftc_cot_raw`, 34 symbols since D572, 1986 → 2026-09, commercial / non-commercial split, which is what Basu–Miffre use — and the disaggregated form is in the same fixture from 2006; **hedging pressure is scored and negative, D573: the sort is the tilt and the tilt lost**; **basis-momentum's forward slice is read and spent, D574: INSIDE, the composition inverted, nothing enters**), **basis-momentum** (Boons–Prado; `BASIS_MOMENTUM.md` is a pre-registration draft and the settlement strip holds every listed month it needs), **commodity skewness** and **commodity value**, in that order of the deposit's own confidence. A name-randomised null (keep the leg counts, randomise who fills them) would separate tilt from timing and none of D557–D562 pre-registered one — D561 and D562 show why it matters (the rotation null's median is +0.11 / +0.18 on the Sharpe and −0.68 / −0.55 σ on ES's worst days, which is tilt). The overlay on the ledger's live arm needs that arm's daily P&L on disk. **The three cross-sectional sorts did not read 2024+ and keep it.**
