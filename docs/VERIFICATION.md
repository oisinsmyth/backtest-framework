...
**3,007 tests are collected here, and one of them skips on the machine this was written on, for
...
### `tests/golden/` — 248 tests. *The arithmetic is right.*
...
### `tests/property/` — 145 tests across 14 files. *The invariants hold on paths nobody chose.*
...
### `tests/unit/` — 2,455 tests. *Each part does its own job.*

The bulk, and the least interesting per test: one behaviour, chosen inputs. This is also where
most of the **structural guard** assertions are proved to fire — `src/` carries **580 `raise`
statements**, spread across 57 of its 95 tracked `.py` files, and a guard nobody has proved will
