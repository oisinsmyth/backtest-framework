...
also names the five panels that have no blob id rather than only counting them. The rest are
...
(2026-09-16). [`data/data_manifest.json`](../data/data_manifest.json) carries the sha256 of all 120
panels and the git blob id of **115** of them. The five without one —
`data/d377_ensemble.npz`, `data/d382_scores.npz`, `data/fixtures/fut_day1m.parquet`, and the two
settlement panels D556 built on 2026-09-19, `data/fixtures/fut_settle_strip.csv.gz` and
`data/fixtures/fut_curve_front_next.csv.gz` — were never tracked in the first place, so the
