...
|---|---|---:|---:|---:|---:|---:|---:|
| Settlement flow ledger (`SETTLEMENT_FLOW_LEDGER_PREREG.md`) | 12. Required unit tests (line 705) | 71 | 29 | 0 | 5 | 37 | 40.8% |
...
Section 12. Required unit tests, heading at line 705, items at lines 707–777. **29 of 71 claimed (40.8%).**

| # | Line | Class | Status | Claim | Item |
|---:|---:|---|---|---|---|
| 1 | 707 | arithmetic | **unclaimed** | — | `Q1` for L = +2, AUM = 1e9, r = 0.05, f_fut = 1 gives +1e8 notional before contract conversion. L = −2 gives +3e8 (the same sign as the move).<br>*Q1 rebalance notional signs and sizes at L = +2 and -2* |
| 2 | 708 | arithmetic | **unclaimed** | — | L = +1 funds contribute zero rebalance flow.<br>*Unlevered funds contribute no rebalance flow* |
| 3 | 709 | arithmetic | **unclaimed** | — | `f_fut = 0` routes all rebalance to P2; `f_fut = 1` routes all to P1.<br>*The futures fraction routes rebalance wholly to P1 or P2* |
| 4 | 710 | leak | **unclaimed** | — | `lag_c = 1`: Q3_known uses ΔShares from t−1 and no forecast term; `lag_c = 0`: the forecast term only.<br>*Creation lag selects realised t-1 shares or the forecast term* |
| 5 | 711 | arithmetic | **unclaimed** | — | Update step: with p = 0 the posterior equals the prior and Q_rem = μ_total; with R → ∞, K → 0.<br>*The update degenerates correctly at p = 0 and infinite noise* |
| 6 | 712 | arithmetic | **unclaimed** | — | Update step: a known synthetic case (μ = 100, σ² = 400, p = 0.5, R = 100, z = 60) matches hand-calculated K, Q_hat, Q_rem.<br>*A hand-calculated gain, posterior and remaining quantity* |
...
| 8 | 714 | leak | **unclaimed** | — | No look-ahead: V_d, σ_d, S_norm and the fund data used on day t exclude data from day t onward (fund data respects `published_at`).<br>*Volume, volatility and fund data on day t exclude day t onward* |
| 9 | 715 | execution | **unclaimed** | — | Earliest-pass rule: signals at the first passing t0 and never re-enters.<br>*Entry fires at the first passing t0 and never re-enters* |
...
| 12 | 718 | data_guard | **unclaimed** | — | Contract mapping: flow is attributed to the held contract months per holdings, not the front month by default.<br>*Flow is attributed to the held contract months, not the front* |
...
| 14 | 720 | arithmetic | **unclaimed** | — | iNAV with zero futures move equals NAV[t−1] × (1 + accruals); with L = 2 and r = +1% it rises about 2%; with L = −2 it falls about 2%.<br>*iNAV arithmetic at zero move and at L = +/-2* |
| 15 | 721 | arithmetic | **unclaimed** | — | Premium uses the NBBO midpoint: a synthetic series with trades at the ask but a constant mid shows zero premium.<br>*Premium is measured off the NBBO midpoint, not trade prices* |
| 16 | 722 | data_guard | **unclaimed** | — | Stale-minute exclusion: an ETF quote not updated for more than 60 s, or a futures minute with no trade, is excluded from all features.<br>*Stale quotes and tradeless futures minutes leave every feature* |
| 17 | 723 | leak | **unclaimed** | — | No premium feature uses any minute at or after W_start.<br>*No premium feature reads a minute at or after the window* |
| 18 | 724 | arithmetic | **unclaimed** | — | h is in (0, 1) and non-increasing in stress when h1 ≥ 0.<br>*The hedged fraction stays in the unit interval and falls with stress* |
| 19 | 725 | arithmetic | **unclaimed** | — | Creation sign: a KOLD (L = −2) creation gives negative Create_flow (the fund sells futures); a BOIL creation gives positive.<br>*Creation flow sign follows the product's leverage sign* |
| 20 | 726 | arithmetic | **unclaimed** | — | Gate 0b check reproduces a hand-calculated iNAV-vs-NAV error for a synthetic fund.<br>*Gate 0b reproduces a hand-calculated iNAV-versus-NAV error* |
| 21 | 727 | leak | **unclaimed** | — | Attention z-scores on day t use only data from days t−60 … t−1 for the same hour-of-day and day-of-week.<br>*Attention z-scores use only the prior sixty matched days* |
| 22 | 728 | leak | **unclaimed** | — | Publication-time guard: an hourly pageview for 13:00–14:00 is unavailable at τ = 14:10 and available at τ = 14:15 or later.<br>*An hourly pageview is unavailable before its publication lag* |
| 23 | 729 | leak | **unclaimed** | — | GDELT items are keyed on publication timestamp; items published after τ are excluded even if their event time is earlier.<br>*GDELT items are keyed on publication, not event, time* |
| 24 | 730 | arithmetic | **unclaimed** | — | `att_accel` on a synthetic step increase in attention is positive during the ramp and returns to about zero once the level is flat.<br>*Attention acceleration is positive on a ramp, zero once flat* |
| 25 | 731 | rendering | **unclaimed** | — | Query lists are loaded from `QUERIES.md` and hashed. The hash is stored with every trial in `trials.csv`.<br>*Query lists are loaded from QUERIES.md, hashed, and logged per trial* |
| 26 | 732 | arithmetic | **unclaimed** | — | P9 formula: a +3× product with AUM $50m and r = +4% gives ΔH = +$12m; a −3× product gives +$24m (same sign as the move).<br>*P9 hedge change signs and sizes for +3x and -3x products* |
| 27 | 733 | leak | **unclaimed** | — | FX conversion uses the rate at the prior settlement, never the current day's.<br>*FX conversion uses the prior settlement's rate* |
| 28 | 734 | data_guard | **unclaimed** | — | P9 flow is mapped to each product's own index contract month (2nd-front for WisdomTree NG, front for BetaPro).<br>*P9 flow maps to each product's own index contract month* |
| 29 | 735 | arithmetic | **unclaimed** | — | Restrike detection: for a +3× product, a synthetic −6.7% underlying move from the prior reset triggers a restrike; −6.5% doesn't. After a restrike, the settlement ΔH is computed from the restrike level.<br>*Restrike triggers at the contractual threshold and resets the level* |
| 30 | 736 | data_guard | **unclaimed** | — | BetaPro L_eff: when published notional is missing for a day, stated L is used and the day is flagged.<br>*Missing published notional falls back to stated leverage and flags* |
...
| 33 | 739 | data_guard | claimed | `tests/unit/test_recorder.py:476` `test_ledger_33_a_missed_daily_window_is_flagged` | Gap detection flags a missed daily job, and no downstream code fills the gap.<br>**Note:** D608. Five test_ledger_33_* functions here and four more in tests/golden/test_recorder_ledger.py. |
...
| 37 | 743 | execution | **unclaimed** | — | UNG roll fractions: four days at 25% each; cumulative 100%; day 1 starts two weeks before near-month expiration per the rule.<br>*UNG roll fractions are four days of 25% starting on the rule's day* |
| 38 | 744 | execution | **unclaimed** | — | Roll sign: a long fund's roll sells the expiring month and buys the next; an inverse fund does the reverse. New-month contracts are scaled by P_old / P_new.<br>*Roll sign and price scaling for long and inverse funds* |
| 39 | 745 | data_guard | **unclaimed** | — | Roll validation: synthetic holdings changing by 25% per day across the window pass; holdings that change all on one day fail and trigger the P8b fallback.<br>*An all-on-one-day holdings change fails and triggers the fallback* |
| 40 | 746 | arithmetic | **unclaimed** | — | No double counting: a fund in P8a contributes zero to P8b.<br>*A fund counted in P8a contributes nothing to P8b* |
| 41 | 747 | leak | **unclaimed** | — | COT availability: a weekly record is usable only at or after its Friday release timestamp.<br>*A weekly COT record is usable only from its Friday release* |
| 42 | 748 | statistical | **unclaimed** | — | The swap-dealer fit constrains λ to [0, 1]; the consistency flag triggers when \|n_flow − (1 − λ̂)\| > 2 combined SE.<br>*The swap-dealer fit is bounded and flags a consistency breach* |
| 43 | 749 | data_guard | **unclaimed** | — | The swap-exposure predictor includes only swap-routed exposure (1 − f_fut) and P9, never futures-held exposure.<br>*The swap predictor excludes futures-held exposure* |
| 44 | 750 | leak | **unclaimed** | — | Swap dissemination records are keyed on dissemination time, not execution time, for any predictive use.<br>*Swap records are keyed on dissemination, not execution, time* |
...
| 47 | 753 | leak | **unclaimed** | — | Large-lot threshold uses the trailing 20 prior days only; trades exactly at L_min count as large.<br>*The large-lot threshold uses the trailing twenty prior days* |
...
| 50 | 756 | leak | **unclaimed** | — | Options open interest by strike is joined point-in-time (the prior day's published values only).<br>*Options open interest is joined at the prior day's publication* |
