...
**Three rounds, nineteen briefs, zero strategies — and eight things the programme was wrong
about.** *(Seven briefs in round 1, six in each of rounds 2 and 3; counted, not estimated.)* That is not a disappointing result reported apologetically; it is the actual yield, and it
...
**A hazard worse than a block.** WebFetch's PDF summariser **fabricated a table of percentages that
does not appear in the source document** (R3, `G3`). **A summariser that invents a number is worse
than one that fails**, because the failure is silent and looks like a reading. **Any figure obtained
through a summariser is WEAKER than `[snippet only]`, not stronger.**
