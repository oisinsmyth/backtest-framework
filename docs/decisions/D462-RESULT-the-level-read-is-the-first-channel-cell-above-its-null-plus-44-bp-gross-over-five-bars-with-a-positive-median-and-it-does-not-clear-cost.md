# RESULT D462 — the level read is the first channel cell above its null: +44 bp gross over five bars with a positive median, on both constructions, and it does not clear cost at that hold

*Runner `scripts/run_d451_grow_trades.py --source hand --rule level`; numbers
`data/d462_hand_level.json` (the hand cell is keyed "GROW" in the file and the null/book lines);
log `temp/d462_full.log`; spec [D462](D462-the-level-read-on-the-hand-cell-in-sample.md).
...
## 1. Per trade, gross bp — HAND (D460's cell) and CAUSAL (D399's), the same rule
...
**Books, hold 5** (D434's rotation null, 300 draws): HAND long +11.7% at Sharpe 0.67, exposure
...
- **This is the first channel cell that is a signal by the programme's criterion**: a positive
  ...
  five direction cells (D434, D450, D451, D452, D461) had none of those three.
...
208 s at 3.0× on 12 workers (tail-heavy, as D461); whole run 779 s.
