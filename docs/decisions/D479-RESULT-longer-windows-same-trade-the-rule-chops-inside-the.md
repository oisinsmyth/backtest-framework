# RESULT D479 — longer windows, the same trade; the rule chops inside the window it was built to keep

*Filename shortened 2026-09-17 ([D540](D540-local-config-a-clone-never-receives.md)); was `D479-RESULT-longer-windows-same-trade-the-rule-chops-inside-the-window-it-was-built-to-keep.md`. The H1 above is the full title.*

*Runner `scripts/run_d478_grow_trades.py` with `--tag D479`; numbers
`data/d479_grow_trades_hysteresis.json`; log `temp/d479_full.log`; spec
[D479](D479-trading-the-grow-right-lines-with-hysteresis-in-sample.md). In-sample, the mining
...
| P2 trades longer and fewer | hold median > 10 bars; n < 63,340 | hold median **7** (D478: 6) — **missed**; n = 37,784 — held |
| P3 sweep still falls | gmin 200 below gmin 0 | +9.1 → −23.1, monotone — held |
| P4 CAUSAL reproduces D478 | +16.8 / −41.9 | identical to the last digit — held |
...
| | GROW, hysteresis + trend-side break (this line) | GROW, D478's line | CAUSAL (D399) |
|---|---|---|---|
...
Three constructions of the channel (D399's pivots, D478's grow-right, D479's grow-right with
hysteresis) and two rules (D476's target-and-trail, the hold-while-trend of D477–D479) now give
