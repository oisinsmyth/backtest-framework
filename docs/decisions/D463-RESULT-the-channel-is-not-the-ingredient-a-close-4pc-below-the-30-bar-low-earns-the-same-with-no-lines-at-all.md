# RESULT D463 — the channel is not the ingredient: a close 4% below the 30-bar low earns the same with no lines at all

*Runner `scripts/run_d451_grow_trades.py` (`--rule dip`, `--rule dip --dip-drawn`,
`--rule level --head 20`); numbers `data/d463_dip_all.json`, `data/d463_dip_drawn.json`,
`data/d463_level_hold20.json`; logs `temp/d463_*.log`; spec
[D463](D463-the-dip-control-and-the-hold-20-null-does-the-channel-floor-add-anything-to-buying-a-sharp-fall.md).
...
| hold | level rule on the channel (D462) | dip, no lines | dip, only where the channel is drawn |
|---|---|---|---|
...
At hold 20 (D462's rule, the null moved there): long +112.8 against p50 +61.1 / p95 +76.9; book
...
- **The floor of the channel is not doing anything.** A close 4% below the previous 30 bars'
  ...
  same.
- **What D462 found is short-horizon reversal after a sharp break of a trading range, in both
...
in every headline cell. Event counts against D462's 15,603: dip-all 11,086, dip-drawn 1,961
