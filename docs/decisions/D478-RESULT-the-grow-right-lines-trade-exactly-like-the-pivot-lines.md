# RESULT D478 — the grow-right lines trade exactly like the pivot lines, and both time their own trades worse than chance

*Filename shortened 2026-09-17 ([D540](D540-local-config-a-clone-never-receives.md)); was `D478-RESULT-the-grow-right-lines-trade-exactly-like-the-pivot-lines-and-both-time-their-own-trades-worse-than-chance.md`. The H1 above is the full title.*

*Runner `scripts/run_d478_grow_trades.py`; numbers `data/d478_grow_trades.json`; log
`temp/d478_full.log`; spec [D478](D478-trading-the-grow-right-lines-in-sample.md). In-sample,
...
| | GROW (D478's lines) | CAUSAL (D399's, D477's arm) |
|---|---|---|
...
pivot construction's cells. CAUSAL reproduced D477 to the last digit (P4). Split-guard
rejections 1,267 (GROW's shorter, denser trades put more of them across a halving).

## 1. The sweep repeats D477's pattern (P3)
...
holding them while the channel says "trend". That is D476's finding again ("the channel traded
...
(D478's audits [O], [F], [I]) — and an identical *trade*. Two constructions, two rules
(D476's target-and-trail, D477/D478's hold-while-trend), four causal cells, all within noise of
...
as D477's loop on all 166,170 real trades, gross within 1.9e-12 bp; [XV] it rejects a one-bar
