# RESULT D476 — the final channel, traded long and short: the long side is zero, the short side is negative, and the book is worse than randomly re-timing its own trades

*Filename shortened 2026-09-17 ([D540](D540-local-config-a-clone-never-receives.md)); was `D476-RESULT-the-channel-traded-is-worse-than-re-timing-its-own-trades.md`. The H1 above is the full title.*

*The runner is `scripts/run_d476_channel_trades.py`; the record it answers is
[D476](D476-trading-the-final-channel-long-and-short-both-lenses.md), committed before it existed.
Numbers: `data/d476_channel_trades.json`. Nothing is admitted and nothing is closed — only the
...
Per D476 §4's falsification clause, stated before the run: *"a side whose gross mean per trade is
...
trailing-stop rule and the target multiple is close to decorative — as D476 §5 said it would be
