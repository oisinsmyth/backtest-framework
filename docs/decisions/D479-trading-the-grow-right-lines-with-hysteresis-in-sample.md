# D479 — trading the grow-right lines with hysteresis and trend-side breaks, in-sample
...
## 1. What changed since D478

D478 found the grow-right construction traded exactly like the pivot lines (long +1.0 ± 3.2 bp,
...
overlapping window. Two changes followed, both on `scripts/d478_causal_grow.py` and both pages:
...
On the twelve page names: 79 runs, 54% of bars covered (D478's line: 128 runs, 62%), windows
at their last bar 33–122 bars, widest 25%, price at most 4% off the nearer line.

## 2. The question, unchanged

D477/D478's rule: **in while there is a trend, out when there is not** — both lines drawn,
...
which must reproduce D478's numbers). Same panel, eligibility, split guard, spread, nulls
(within-name time rotation of the state series per trade, 200 draws; D476's book rotation null,
300 draws) and audits ([V], [XV], [F], [S], [M], [N], [SPEED]) as D478.
...
- P1. GROW long at `gmin` 25: gross mean per trade between −20 and +30 bp; short negative; neither
  side above its per-trade null p95 by 2 SE. The mechanism D477 named — a confirmed direction is
  a late one — does not depend on how long the window lives.
- P2. The trades are longer and fewer: GROW long median hold above 10 bars (D478: 6), and fewer
...
- P4. CAUSAL reproduces D478 exactly: long +16.8, short −41.9 at `gmin` ≤ 25.
