# D461 — trading the hand cell, in-sample: does a construction that draws the principal's lines change what their direction is worth?

*Pre-registration, written before the runner change (R8). Runner: D451's
`scripts/run_d451_grow_trades.py` with `--source hand`, output `data/d461_hand_trades.json`.
Sequel to [D460](D460-RESULT-the-hand-cell-the-pivot-construction-re-dialled-reaches-the-eye-on-gradient-and-the-swing-envelope-loses.md).
In-sample only, the mining panel, 1,573 names; no holdout is read.*

## 1. What is traded

The **hand cell** of D460: D399's pivot construction re-dialled against the principal's 140
...
The rule is D450's, unchanged: **in while there is a trend, out when there is not** — both lines
...
reproduce D451/D452 (+16.8 / −41.9 at `gmin` ≤ 25). Same eligibility, split guard, spread,
...
- P1. HAND long at `gmin` 25: gross mean per trade between −20 and +30 bp; short negative; neither
  side above its per-trade null p95 by 2 SE. (D450's mechanism — a confirmed direction is a late
