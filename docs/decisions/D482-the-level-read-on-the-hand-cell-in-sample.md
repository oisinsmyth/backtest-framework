# D482 — the level read on the hand cell, in-sample: the one reading of the channel none of the five cells made
...
inside the hold extends it (the same run-extraction as D477's rule, on a different state
series). H swept over {1, 3, 5, 10, 20}, headline 5. Both sides on the same bar: flat.

Two sources, the same run: **HAND** (D480's cell) and **CAUSAL** (D399's `CELL_FINAL`). Same
...
closed in writing — direction (D476, D477, D478, D479, D481) and level (D482) both read, on a
