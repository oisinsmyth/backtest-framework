...
## 77. The mechanism programme, D577–D581, and the closure of the deposit's list (D582): the three mechanisms with a clock and a sample in sessions each failed its own declared bar, and each failure named a specific error

**The deposit's fourteen files are now every one run or killed** ([D582](decisions/D582-CLOSED-the-deposit-and-the-mechanism-programme-synthesis.md), closed by the principal 2026-09-21). After the curve (section 76), three mechanisms with a clock remained and had data on disk or free: producer hedging flow in the weekly COT, the perpetual funding cycle, and dealer gamma at the close.

**Hedging flow (D577, D578).** On the declared statistic neither COT category responds to the 12–24-month strip (swap dealers +18k contracts a log-point, t 1.2). The net short, reported beside, responded at +71k (t 3.7) in sample and was pre-registered forward on 139 unread reports: **−32k, t −1.1, rank 0.11**, the point estimate rejecting the in-sample beta at z −3.7. The post-mortem found why inside the in-sample window: the rolling three-year beta ran +115k to +161k through 2019 and was zero at end-2023, and the swap-dealer gross short that carried it fell from 184k to 32k contracts. **A pooled regression β taken forward is the average of its regimes; its rolling path and its instrument's level path are reported before it is pre-registered, or the forward test tests the average of the dead and the living.**

**The funding clock (D579, D580).** The clock is on CME bitcoin — volume 1.9× and variance 1.8× the cycle mean at the settlement minute, for five minutes, peak at minute 0 — but the declared sixty-minute window dilutes it to 1.10× (rank 0.94 / 0.87 in 96 placements, bar 0.95), the burst holds 2018–2021 only, and the funding sign orders nothing (−1.0 bp, SE 1.5, +4.8 bp in 2021 and −9.6 in 2022). Two method facts: **a contract that prints a bar only when it trades needs presence defined by the session, or a bar-count rule is a liquidity filter** (the literal rule kept 1,215 of 6,611 settlements); and **an event window sized before the profile is seen must be reported with the profile at its own resolution and the overlapping placements named**.

**Carried dealer gamma (D581).** With every ES-family option's open interest by strike (19.2M rows, 25 families, from two zero-cost pulls — `ES.OPT` is the quarterly family alone), the interaction of the last half-hour's return with the prior hour and the regime sign is **−0.023 at t −0.66, rank 0.32** on 1,990 sessions; the close continues its prior hour in every regime (b +0.10, t 2.9, D463's effect); what little regime dependence exists sits on sessions without a same-day expiry. Carried open interest sees at most 8.5 % of same-day gamma, as the deposit's own premise correction said.

**Parked, with numbers, not components:** the close's continuation of its hour (cost-bound at K3); the five-minute funding burst (unsigned); producer/merchant longs buying crude rallies in both samples (+68k and +241k contracts a log-point). **Where next is the principal's:** a purchase that opens a line not on this list, or a new deposit. Norgate extends only closed lines; signed option flow serves the mechanism whose cheap test just failed.

## 78. The basis-momentum programme, D564–D602, and the closure of `BASIS_MOMENTUM.md` (D603): ten free mechanism tests in four Stage 0 records, and not one prediction supported — the curve moves together, the dealer factor reads zero, storage is in the signal, and the FX extension is void on its own harness

**The document is closed by the principal** ([D603](decisions/D603-CLOSED-basis-momentum-every-free-test-run-none-supported.md), 2026-09-21), after the in-sample replication (section 76; +0.42 as pre-registered, +0.69 only under a formation amendment made after the read), the forward read (D574: +0.48, inside, the seasonal roots that carried it lost), and every mechanism test the deposit names that needs no purchase.

**The calendar, liquidity and volatility (D583, D584).** The ten sessions into a quarter-end earn −0.37 bp/day *less* than the rest (rank 0.47 in 63 enumerated placements; the quarter-ends 2.7 bp/day below the other month-ends). The eight least liquid roots earn more than the nine most liquid (+5.46 bp/root-day) at rank 0.85 among the 24,310 enumerated splits, with a Spearman of +0.02 where the mechanism needs a negative, all of it on the two roots the book holds least. The 21 post-spike sessions earn −0.57 bp/day less. The commercial-positioning control holds on its meaning (−0.15 with HP) and fails its letter only on the same-week signed flow (−0.35), which is the commercials-against-price relation every root shows.

**The curve, the season, the dealer and the store (D600).** BM(2,3) — the same construction one contract out — earns +0.37 at rank 0.90 and curvature −0.08; the decisive number is the primary's own membership scored on the *second* nearby's return, **+0.70, 94 % of its P&L**: nothing is maturity-specific, the curve moves together. The de-seasonalised signal survives its rotation null at ρ 0.89–0.93 with the raw one, because **a twelve-month sum of a fixed calendar effect is a per-root constant** — arithmetic, not evidence. The cross-sectional IC is real (+0.048 at t 2.2, rising to +0.11 at twelve months, 9 of 12 years) and was already read forward as a book, inside. The book loads **+0.006 on the He–Kelly–Manela capital risk factor, t 0.13**, and earns 81 bp/month *less* after the lowest capital tercile. On the five energy roots the signal is **negatively correlated with the de-seasonalised inventory surprise on every root (−0.13 to −0.48)**: the storage state the paper calls the premium inconsistent with is in the signal. The COMEX metals are held on their inactive months (PL 25 lots a day at the second nearby): **the listed delivery is not the traded one.**

**FX in covered-parity space (D602).** In logs, basis momentum is minus the cumulated change of the log calendar spread, whose parity fair value is the tenor-weighted rate differential with spot cancelling — so on raw settlements FX basis momentum is minus a quarter of the twelve-month change in the 3-month rate differential (the deposit's objection made exact; observed −0.514 against a bar of −0.5). The correction from a single monthly 3-month tenor over-shoots (**+0.598** against a bar of 0.3): the curve prices the expected policy path, the correction has the level. Void, not failed; the one cell that clears a null is rate momentum wearing the signal's name; the FX 2024+ slice stays unread.

**Not run, on the ruling:** M5 on the NASS roots (a failed control on a spent slice can only be made more precise), a re-specified FX construction (forward or OIS rates, paid, for a signal with no support elsewhere), T1 (not runnable). **Bought:** the macro fetcher and four gated fixtures, the third-nearby strip, the covered-parity construction, and the answer to the deposit's own selection criterion: chosen for orthogonality to hedging pressure, the signal is orthogonal to hedging pressure, correlated with storage, and carries no premium the nulls cannot produce. **Where next is the principal's**, as D582 §9 said: a purchase that opens a line not on the list, or a new deposit.

## 79. Expiry pinning on ES from the same-day option ladder (D613/D614): the pull is ten sessions of 1,132, it changes sign with the standard error, it is absent in the era 0DTE dominates, and a conditioner with no option content clears the same rank bar

**Not pinning, and the line closes** ([D614 result](decisions/D614-STAGE-0-RESULT-not-pinning-ten-sessions-carry-the-whole-pull.md), 2026-09-22; [pre-registration](decisions/D614-PRE-REG-expiry-pinning-on-ES-from-the-noon-0DTE-volume-ladder.md); fixture [D613](decisions/D613-FIXTURE-the-ES-option-volume-panel-in-ET-clock-buckets.md)). The first candidate of the post-deposit programme, and the first built on a regularity observable in data already held rather than on a paper.

**The construction.** The volume-weighted centroid of same-day-expiring ES option volume accumulated to **12:00 ET**, signed against the 15:30 price and scaled by the trailing sigma of the 15:30 → 16:00 window, regressed on that window's return with the overnight, the 09:30 → 12:00, the 12:00 → 14:30 and the 14:30 → 15:30 moves and a grid-only placebo as fixed controls. A noon cutoff because a conditioner accumulating to 15:30 is contemporaneous with the window it predicts; 61.9 % of the pre-15:30 volume is in by noon, and the two ladders correlate **0.979**, so the cutoff bought less independence than hoped and the prior was set low accordingly.

**Why the coefficient is not an effect.** It is **−5.08 bp per sigma** — repulsion, where the mechanism predicts attraction — at Newey-West t −2.11 under the pre-registered estimator. But **White's leverage-adjusted HC3 gives −1.96 and Newey-West lag 10 gives −1.84**, and the gap from the homoskedastic 0.59 to the robust 2.41 is heteroskedasticity, not autocorrelation. **Ten sessions of 1,132 hold 78.7 % of the slope’s numerator, and three of them hold 81 %: 2020-02-28, 2020-03-13 and 2020-03-02, the COVID crash.** A slope weights each row by its own extremeness times its outcome, and those rows are 6.1× typical in distance and 7.6× in close move, so they count many times over; dropping the ten takes the coefficient from −5.078 to −1.189. Every one of the ten is a crisis afternoon — the 2016 election, volmageddon, the Christmas 2018 bottom, the 2020 crash — with the ladder far below the price and the close rallying hard. The trades do not cluster at zero, which is the other misreading available: only 1.6 % capture nothing and 12.4 % fall inside one round trip, the quartiles being −$22 and +$20. By tercile of distance the coefficient runs −1.71, **+0.11**, −7.20, so it is non-monotone and largest where the centroid is *furthest* — a hedging pull is strongest near a strike, not far from it. Before 2022-05 it is −5.73 and after it **−0.16**, which is when the Tuesday and Thursday families filled the week and 0DTE became the dominant flow. The two mechanically motivated strike bands sit inside their nulls at 0.905 and 0.453. And the **sign-flip null, the only one that keeps each session's distance paired with its own volatility, puts the observed at 0.956** where the two rotation nulls say 1.000, because rotation decouples the regressor from the residuals it co-moves with.

**Three method facts worth more than the verdict.** *(i)* **A rank bar can be clearable with no information in it.** The equal-weighted centroid of the same listed strikes, carrying no volume at all, reached rank **0.987**; it failed the pre-registered conjunction only because every robust t on it is under 2. A band centred on the current price over a five-point ladder carries a sawtooth remainder of the price modulo the step, worth 0.38 sigma pre-2022 against a mean signal of 2.6 — so the placebo has to be an arm that must fail, not an afterthought. *(ii)* **A rotation null of a regressor that is correlated with its controls is anti-conservative**: rolling it destroys the correlation, the null's design becomes better conditioned than the observed one, and its spread came in at 0.298 against a fitted 0.586. Residualise on the controls first and roll what is left, which preserves the regressor's norm exactly. *(iii)* **A design that carries one variable twice does not fail, it explodes** — the placebo cell held the grid centroid as a control and as its own target and returned a null p95 of 6.4e13. A guard that raises when any two columns correlate above 1 − 1e-8 now runs on every cell.

**Also established, and reusable:** the coefficient `c_only` returns in `stage0_d581_gamma_close.py` is the **interaction**, not the level, because `add_constant` prepends — verified on a level-only panel where the level reads +5.09 and the positional read returns −0.01. Any study reusing that helper for a level claim is scoring a different hypothesis. And **D409's Dg1 guard cannot fire** on a Herfindahl concentration whose floor moves with the strike count: it returns +0.05 on every variant including a floor-only placebo. Replaced by correlation guards against the trailing sigma and against 1/n, which do fire — on exactly the measures the review said were contaminated, leaving the fixed-count measure at **+0.067 against a standard error of 0.164**, that is, nothing.

**Component line:** one MES in at 15:30 and flat at 16:00 loses in both directions — attraction −$6.74 a session net, repulsion −$1.76 — against a $4.25 round trip and a 1.21 bp breakeven a side, with ρ −0.14 to the admitted MACD arm. The regression's 13.3 bp expected move is magnitude-weighted and lives in the tail; a sign-only book, which is all one lot can express, realises 1.16 bp of gross move and the median session earns exactly minus the cost. **What a real test would need** is the aggressor-flagged flow that signs who bought the 0DTE contracts, which is inside the reserved year, or a market where position rather than turnover is observable.

## 80. The six sharpenings of the 0DTE ladder (D616–D618): the axis closes on two independent legs, the "interior solution" that appeared was the band being centred on the price it predicts from, and a screen that rejected a perfect oracle nearly wrote the wrong verdict

**A SCREEN IS A MEASURING INSTRUMENT AND NEEDS A KNOWN-ANSWER CASE BEFORE IT IS TRUSTED.** This section's first version reported "of 72 cells not one passes the construction screens". That was wrong, and the principal's question — were the screens too harsh — is what found it. Screen S1b required `sd(LADDER) >= 3 x sd` of the same cell with **flat weights over its positive-weight strikes**, and it **rejected a conditioner built from the actual 16:00 close**: perfect information, scored 0.05. For any smooth weight the positive-weight set is the whole 220-strike ladder, whose centroid sits ~23σ from the price, so the ratio asks "is this as dispersed as the entire ladder" and penalises exactly the concentration a real signal has. A within-session permutation comparator inverts the same way (oracle 0.04, grid exactly 1.00), because a reshuffle scatters weight across the ladder and every informative object is *narrower* than its own permutations. The well-posed form of the question is `|corr(LADDER, PING)| <= 0.50`, where the grid control **is** PING and scores 1.0000 by construction while oracle, diluted and weak oracles score 0.0605, 0.0359 and 0.0196 — a threshold read off the controls and never off the real cells. **The screen had been stated plausibly, passed a synthetic unit test, rejected everything, and was invalid.** Any gate that *rejects* rather than *reports* should be shown to admit an oracle before its rejections are believed.

**The verdict, corrected.** Of **72 cells**, **five pass** the construction screens — including `gamma_doi|all|w30|now`, the pre-registered priority cell #1 — and **none clears its own nulls**. The correction runs *against* the convenient answer: it took the scoreable set from 0 to 5 and gave the hypothesis more chances, not fewer. The screens are dispersion, distinguishability from the grid centroid, independence of the day's move, freedom from sawtooth, and strike support: statements about the conditioner alone, which is why a spent window cannot corrupt them.

**The pre-registered primary has the predicted sign and no effect:** +1.663 (attraction, where D614 read −5.08), NW t **+0.96**, rank 0.757 in its own shift null and 0.508 in the sign-flip null, expected move **$0.66 against a $4.25 round trip**, tail bar held so it is not a crisis-day artefact. Family maximum |t| **1.961** against a family-max null p95 of **2.714** while each cell's own p95 runs 1.86–2.13, so the largest t in the family does not reach even a single-cell bar.

**The lesson worth carrying, and it cost a full false positive to find.** The runner's first pass centred each band on **P1530**, which the pre-registration names as the *break* rather than the specification. Four cells passed every screen and one cleared both its own nulls (shift rank 0.966, flip rank 0.974). Re-anchoring the band on the **prior settlement**, where the record says it belongs, moves those same cells from sd 0.62–1.04σ to **5.57–5.59σ**, from corr(DAY0) −0.19/−0.21 to **−0.54**, and from **3.05–3.54× their flat-weight placebo to 1.00×**. Selecting the strike set around the current price manufactured *both* the independence and the apparent weight information. **A band is a selection, and a selection centred on the outcome's own starting price is the grid artefact one level up.** The two bands disagree on **90.7 %** of sessions, median Jaccard **0.333**, so they are not variants of one object.

**Where the dispersion actually comes from.** An adversarial review had concluded that dispersion in a signed distance to a weighted strike comes either from the grid's relation to the price (no information) or from the day's own move (−0.55), with no interior solution. Both legs are confirmed: every pre-session-anchored weight reads corr(DAY0) between **−0.48 and −0.54** against the morning leg and **−0.73 to −0.74** against the full 09:30 → 15:30 move, D614's own volume centroid among them; and every *banded* cell reads corr with its own grid centroid between **+0.998 and +1.000**, because with a median of 6 strikes in a ±3-step band the weighted centroid is the band's midpoint. The prediction that no cell escapes both is nonetheless **falsified**: the Δ-OI centroid unbanded is wide (**13.1σ** on the half-hour, **19.0σ** on the ten minutes) *and* independent (**−0.134**, **−0.142**, grid R² 0.003) *and* distinguishable from the grid (+0.13). It escapes the construction objection and then reads **−0.0006 at t −0.01** on the close. That is the honest route to the same conclusion — settled by scoring the cell rather than by inferring from a ratio, which is what the first version of this section did with a statistic that turned out to be malformed.

**And a dispersion floor cannot stand in for an economic bar at all.** Setting one honestly from D614's measured economics (+$2.49 gross a session at sd 3.07σ, needing 1.71× to cover $4.25) shows that clearing cost at that coefficient would take **sd ≈ 5.2σ** — *more* dispersion than the construction that already failed. A cell must beat the coefficient, not match the range, and that is unknowable before the outcome is read. Keep the measurement screen and the economic bar apart.

**The nulls were then put through the same discipline as the screens, and they hold.** Power: a conditioner weighted on the strike nearest the **actual 16:00 close** clears both nulls (|c| 25.18 against a shift p95 of 1.43), and so do diluted versions at 30 %, 15 % and **8 %** of the move — so neither null is too harsh, and the rejections are real. Size: 40 standard-normal conditioners clear at **1 of 40 (2.5 %)** in each null against a nominal 5 %, consistent at that sample and erring conservative. The shift null is **enumerated** rather than sampled, so its p95 carries no sampling error and D373's two-SE margin rule does not apply to it. **Validate a null by its power against an oracle and its size against noise — a null that rejects everything is as suspect as a screen that does.**

**The ceiling on this clock, which is the reusable number.** A *perfect* sign-predictor on 15:30 → 16:00 earns **$30.65** gross per trade at one MES (median $17.50), and **$21.41** on 15:50 → 16:00. So breaking even against a $4.25 round trip needs **13.9 %** of the available absolute move on the half-hour and **19.9 %** on the ten minutes. Swept over **126 cells** including every endogenous-band variant, **exactly one clears gross** — `vol_1530|3step|w30|ENDOGENOUS_BAND` at **+$4.65 a trade, t 2.90, gross Sharpe +1.317**, and *not* tail-carried (1 %-trimmed mean $4.58). **It is last-hour momentum wearing a ladder costume, and that was measured rather than assumed:** a plain `F5` sign book (14:30 → 15:30) earns **+$3.20 at t 2.00**, the two signs agree on **65.2 %** of sessions with signed-P&L correlation **+0.458**, the conditioner correlates −0.173 with `F5`, and on the 425 sessions where the two disagree the cell earns $2.17 at about **t 0.8** — nothing. The mechanism is transparent: volume-to-15:30 accumulates **where the price has been**, so the near-money volume centroid lags spot, and the earning arm (*repulsion*) is **long after a rally** — D463's continuation, already in the ledger, and itself short of the $4.25 cost. **The general lesson: a sign-only book with no controls can show Sharpe 1.3 at t 2.9 while the same conditioner's controlled coefficient reads t −0.80 inside both nulls. Score the book AND the coefficient, and when they disagree, name what the controls removed.** The best *clean* construction captures **9.3 %** of the move. **So this window is not too small to pay: the constructions are short by a factor of about 1.5, not by an order of magnitude.** A conditioner capturing one move in seven would break even at minimum size.

**What looking at a family costs, measured.** On the specified family: max |t| **1.961** against a family-max null p95 of **2.714** while each cell's own p95 runs **1.86–2.13**, so five correlated cells cost about **0.6 of t**. On the endogenous-band diagnostic, where one cell did clear its individual nulls: max |t| **2.336** against p95 **2.608** (rank 0.920) with per-cell bars 1.93–2.04 — the multiplicity gap alone sank it. An empirical best-of-N floor beats any correction formula here because the weights are 0.75-correlated.

**Open interest needed a reference date before it could be differenced.** D616 added `oi_ref_session` from the `ts_ref` already in the cached statistics — no pull, and the twelve original columns byte-for-byte unchanged, so D581 and D614 still reproduce. Of 12,594,319 option rows, **339,359 were refused because the two reference sessions were not adjacent against 31,589 refused by the session calendar alone**: ten times as many bad pairs were invisible to the calendar, because a publication is usable on exactly one session so publication times never repeat (0.0000) and CME's preliminary-then-final revision cannot be seen in them. The revision rate rises with the era — **0.24 % of adjacent pairs in 2016, 2.53 % in 2025** — so the reserved year is the worst year for any Δ-OI construction. The zero-delta share is **0.4850** and, against that revision rate, those zeros are genuine.

**Also established.** The aggressor side on ES options is clean where it was feared dirty: unsigned share **0.000166** of 162.8M contracts, at-quote agreement **0.999983** on 22.5M trades — 389 disagreements, which is what an *independent* check looks like, where exactly 1.0 would have suggested the quote snapshot was derived from the trade. D581's gamma audit compares a **total** and is therefore permutation-invariant, so it cannot catch a strike-mapping bug; the audit a per-strike reduction needs permutes the weights *within* the session and requires the scalar to move, and its break is a reduction that returns a plausible number with the pairing lost. **sd(R10)/sd(R30) = 0.6584**: the last ten minutes carry 43 % of the half-hour's variance, so shortening the window leaves `t` untouched and costs a third of the gross. And a null must compare like with like — comparing a **signed** observed coefficient against an **absolute** null distribution makes every rank 0.000 by construction, which is what the first pass of this runner reported for all four cells.

## 81. The intraday clock is market structure and not a convention: the fee's share of the hourly move varies FOUR-FOLD across the ES session, and the hours that carry hour-to-hour predictability are not the hours you can afford to trade

**Why this was measured.** Asked whether past studies would have done better with trades restricted to the first and last hour of the day. Re-scoring closed lines under a time-of-day filter is N new looks on spent windows and was declined — it is the mechanism that produced this session's one false positive. The *premise* is answerable without spending anything, as a census over the committed hourly session table (D467): ES, 2,040 sessions, 2016-01-04 → 2023-12-29, **no strategy scored and no line reopened**.

**The fee's share of the hourly move, per clock hour, at one MES against a 4.25-dollar round trip.** It runs from **0.11** at 10:00–11:00 ET to **0.45** at 23:00–00:00 — a **four-fold** spread, and the most consequential number here for a cost-constrained account. The cheapest hours are exactly the ones the question named: **10:00–11:00 at 0.11, 15:00–16:00 at 0.12, 09:00–10:00 at 0.13, 14:00–15:00 at 0.15**, against 0.16–0.17 at midday and an overnight average of **0.30**. Mean absolute hourly move in dollars at one MES: **37.43** at 10:00, **36.19** at 15:00, **33.71** at 09:00, against **9.52** at 23:00. Median volume: 189,000–229,000 contracts in the cash hours against 4,000–10,000 overnight. **An hour is therefore not an arbitrary slice of the day: the same signal surrenders three times more of its gross to the fee overnight than at the open or the close.**

**The twist, which is the finding: structure and tradeability live in different hours.** The correlation of an hour's return with the NEXT hour's is essentially **zero in the cheap hours** — 09:00 **−0.040**, 10:00 **+0.034**, 15:00 **−0.018** — while the largest values on the clock sit in the thin ones: **+0.167** at 21:00–22:00, **−0.181** at 06:00–07:00, **−0.143** at 23:00–00:00, **−0.128** at 07:00–08:00. At n = 2,039 a correlation's standard error is 0.022, so those are |t| of 6–8 and clear a best-of-22 bar comfortably: **the overnight hour-to-hour structure is real.** It is also unaffordable. A sign book on correlation rho earns roughly rho times the mean absolute move, which is **2.98 dollars** at 06:00–07:00 and **1.98** at 21:00–22:00 against a **4.25** fee.

**The product of structure and size peaks in exactly one place on the clock, and it is the place this programme stumbled into.** 14:00–15:00 predicting 15:00–16:00: rho **+0.113** against a mean absolute move of **36.19 dollars** implies **4.09** against a **4.25** round trip — break-even, reached independently of §80's measurement of 3.20–3.40 on the half-hour version and consistent with it. **Across the whole ES session there is one hour pair where predictability and size multiply to something near the fee, and it lands just under it.**

**How to use this, and how not to.** Not by re-filtering closed studies, which spends looks on a spent window to answer a question the filter was chosen to answer. The legitimate use is forward: **a time-of-day dimension belongs in the pre-registration of any future intraday construction, declared before the run**, because the fee share is a known four-fold lever and the autocorrelation structure is a known map. The 22 hour-pair correlations above are disclosed looks on the in-sample window.


## 82. The signed closing-flow census (D623): the retail crowd IS in the last hour after a decline — about ten per cent more of it than normal — and its flow is BALANCED, so the stop-cascade and inventory-discharge readings of D622's volume hump are both excluded

D622 established that something is specific to ES 14:00 → 15:00 → 16:00 and could not say what. The principal named the counterparty and the mechanism — retail traders, a stop-hunting cascade — and that framing is testable on FLOW rather than on price, where one session gives thousands of observations instead of one. The census read `data/fixtures/fut_micro_flow_5m.csv.gz` over 2025-09-11 → 2026-06-30 (226,224 buckets, 207 sessions, D485's reserve from 2026-07-01 left unread) and scored no return.

**The premise does not hold.** On the 26 arm sessions the 15:00–16:00 signed imbalance `(sell − buy)/(sell + buy)` is **+0.00092 on ES at t +0.21**, its mean flips sign against its median, and **exactly 50.0 %** of arm sessions are net sell-initiated. The enumerated rotation null of the arm label over 150 offsets puts the observation at **rank 0.467** — below its own null's median. Because a symmetry claim cannot be established by failing to reject, the census reports what the data EXCLUDE: against a material effect declared as half the cross-session standard deviation on non-arm sessions, **three of four roots exclude a material one-sided imbalance** (ES upper 2-SE bound +0.0097 against +0.0151). Nothing accelerates into the deadline either: every bucket-index slope is zero within noise and the largest last-two-minus-first-two contrast is t +1.94.

**But the crowd is there.** The ≤2-lot share of closing-hour volume is higher on arm sessions on all four roots at t +2.1 to +3.3 — and the same margin exists at MIDDAY, so the level is a session-level characteristic and not a deadline. The difference-in-differences is what survives, and it is positive on **all four roots by two independent measures**, at 2 SE on three: the share DiD reads +0.0206/+0.0280/+0.0175/+0.0118 (t +2.52/+2.58/+2.26/+1.70) and the excess growth of small-lot contracts over the whole book reads +0.0958/+0.1131/+0.0270/+0.0249 (t +3.20/+3.02/+2.41/+1.83). The volume decomposition says what that means: on ES arm sessions the closing hour runs **2.02× midday volume per minute** while small-lot volume runs **1.61×**, so small lots still grow 20 % LESS than the book — the close remains more institutional than midday, just **about ten per cent less institutional than it would normally be**.

**And the crowd is not the seller.** Where the micro contract differs from its mini it differs toward BUYING: MNQ − NQ reads **−0.0085 at t −2.27** on arm sessions, paired within session, and the difference-in-differences against non-arm sessions is unresolved — so the gap is a permanent property of the micro contract, not something a decline produces. That is the retail-flow pattern this programme already recorded (with the body, against only the extreme) and the opposite of a stop cascade.

**Three things this census establishes about method, each of which changed a verdict in it.** (i) **A level margin needs the same comparison on a window the claim is not about** — Q3's four passes survived only as a difference-in-differences once the midday control existed, and the control's own margin reached 2 SE on NQ. (ii) **A share cannot say whether its numerator grew or its denominator shrank**, and the two readings are opposite; the logged volume ratios separate them and they must be per-bucket, because midday is three hours against the close's one. (iii) **The control must be sessions that were measured and did not qualify.** The first pass put the 30 sessions of the σ warm-up in the non-arm group, which made the control partly a calendar filter; fixing it moved MNQ from t +2.16 to +1.83, from pass to unresolved.

**A limit of the fixture, recorded because it is the only thing that would reopen the axis.** `lot1` and `lot_le2` are volume in small trades and are **not split by aggressor side**, so the small-lot share of *sell-initiated* volume — the quantity the pre-registration asked for — does not exist on disk and cannot be recovered. Resolving who sells would take rebuilding the panel from the `tbbo` tape with a (side × lot-bucket) cross. That is a builder change on data already paid for, and it is the only remaining step on this axis that adds information rather than another look. **103 comparisons are disclosed; the census is read on the shape of the answers across four roots and two lenses, never on a single margin.**
