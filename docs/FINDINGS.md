...
## 80. The six sharpenings of the 0DTE ladder (D616–D618): the axis closes on a construction argument, and the "interior solution" that appeared was the band being centred on the price it predicts from

**The verdict.** Of **72 cells** — five weights crossed with four bands, two windows and two gamma anchors — **not one passes the four pre-registered construction screens**, so **no return was scored for any cell in the specified family**. The screens are dispersion, independence of the day's move, freedom from grid sawtooth, and strike support: statements about the conditioner alone, which is why a spent window cannot corrupt them and why they are the study rather than a preliminary to it.
...
**Three sources of variance, not two, and the third needed its own screen.** An adversarial review had concluded that dispersion in a signed distance to a weighted strike comes either from the grid's relation to the price (no information, 0.14σ) or from the day's own move (−0.55), with no interior solution. The run confirms both and adds a third: the **listed ladder's extent**. The Δ-OI-weighted centroid is genuinely wide (**sd 13.1σ on the half-hour, 19.0σ on the ten minutes**) and genuinely independent (**−0.134**, **−0.142**, grid R² 0.003) — it falsifies the pre-registered prediction on that prediction's literal terms — and its **flat-weight ratio is 1.52**, so the width is where open interest lives (out in the tail puts, hundreds of points away), not how much of it sits anywhere. **An absolute dispersion floor would have passed a thirteen-sigma cell and called it signal; only the placebo-relative floor catches it.** Score every weighted-centroid construction against the unweighted centroid of the same strikes before scoring it against anything else.
...
**The multiplicity bar did its job on the artefact.** Family maximum |t| **2.336** against a family-max null p95 of **2.608** (rank 0.920), while each cell's own p95 ran **1.93–2.04**: looking at four correlated cells costs about **0.57 of t**, enough on its own to sink the one cell that cleared its individual nulls. An empirical best-of-N floor beats any correction formula here because the cells are 0.75-correlated.
