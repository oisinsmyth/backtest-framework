"""C4 lane fetcher. Pace 0.35s. Writes bytes to disk, reports status/size/content-type."""
import sys, os, time, urllib.request, urllib.error, hashlib

UA = "C4-research/1.0 (research@backtest-framework.org)"
OUT = os.path.dirname(os.path.abspath(__file__))


def get(url, name=None, pace=0.35, headers=None):
    name = name or ("C4_" + url.rsplit("/", 1)[-1].split("?")[0])
