...
| **D387: the density line has its SIGNAL test at last, and there is no signal** | 0 of 18 cells clear; gross is positive at long holds (88.1 bp) but **net is negative in all 18** against a measured 142 bp spread. **The edge is short-horizon reversion from `x` ALONE** — a time-rotated density does as well or better, so the event-density conditioner adds nothing. **Close condition MET; the decision is the PRINCIPAL'S (R15).** [FINDINGS §55](docs/FINDINGS.md) |
