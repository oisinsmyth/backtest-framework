...
on the twelve names; the drawings are the labelled set every causal construction is scored against.
    ...
    uv run python scripts/d478_splice_draw_page.py           (reuses temp/d399_live_bars.json)

WHY. Every construction so far -- D399's pivots, D478's grow-right, D479's hysteresis -- was
...
OUT = (Path(sys.argv[1]).resolve() if len(sys.argv) > 1 else REPO / "temp" / "d478_draw_page.html")
