"""THE GROW-RIGHT PAGE: D478's causal construction alone, every one of ITS dials, twelve names.

    uv run python scripts/d478_splice_causal_page.py            (reuses temp/d399_live_bars.json)

The oracle page (D477, `d477_splice_oracle_page.py`) carries four hindsight splitters and the
...
`splitChain` (the oracle's restart made online, kept for comparison). `scripts/d478_causal_grow.py`
...
OUT = (Path(sys.argv[1]).resolve() if len(sys.argv) > 1 else REPO / "temp" / "d478_causal_page.html")
...
<div class="wrap">
  <div class="eyebrow">D478 &middot; the causal construction rebuilt from the oracle</div>
  <h1>Grow right</h1>
