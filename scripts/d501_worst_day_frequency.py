"""D501 -- ADDENDUM to D500: is the worst day a one-off or a habit?

    uv run python scripts/d501_worst_day_frequency.py --self-test
    uv run python scripts/d501_worst_day_frequency.py --run [--json]

**AN ADDENDUM, NOT A NEW STUDY.** Same cell, same window, same cost line as D500. It reads one
statistic family off a series D500 already computed: the FREQUENCY and CLUSTERING of large loss
days, with the days NAMED. No search, no new grid, no new signal. Nothing opened, closed or
admitted (R15).

WHY THIS EXISTS
---------------
D500 reported a worst day of -$1,315 against P3's $1,000 cap and stopped at the binary. The
...
eight years is a tail draw, one a quarter is a property of the construction. D500 never named
...
OUT = REPO / "data" / "d501_worst_day_frequency.json"
...
def do_run(as_json: bool) -> int:
    ...
    P("D501 -- ADDENDUM to D500: is the worst day a one-off or a habit?\n")
    ...
    art = {"generated_utc": datetime.now(timezone.utc).isoformat(timespec="seconds"),
           "purpose": "D501: ADDENDUM to D500 -- the RATE of large loss days, with the days "
