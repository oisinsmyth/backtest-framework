"""D480 -- THE SWING ENVELOPE: the construction rebuilt from the principal's hand-drawn lines.

    uv run python scripts/d480_swing_envelope.py --score               sweep the dials against the hand set
    uv run python scripts/d480_swing_envelope.py --score --line "..."  one line, the full card
    uv run python scripts/d480_swing_envelope.py --proof               audits

WHAT THE HAND LINES ARE (data/d478_hand_drawn_lines.json, 140 lines, drawn one bar at a time
...
OUT = REPO / "data" / "d480_swing_scorecard.json"
...
def main() -> int:
    ...
    if a.sweep:
        ...
        OUT.write_text(json.dumps(dict(what="D480 swing envelope: dial sweep scored against the hand-drawn lines",
