"""D480 -- THE HAND CELL: D399's pivot construction re-dialled against the principal's hand-drawn
lines, and the twelve-name page that shows both.

    uv run python scripts/d480_hand_cell.py            score the cell, write the chart JSON
    uv run python scripts/d399_splice_sample_page.py temp/d480_hand_chart.json temp/d480_hand_page.html

WHAT IT IS. The swing-envelope rebuild (`d480_swing_envelope.py`) lost on the scorecard: two-point
...
HAND = REPO / "data" / "d478_hand_drawn_lines.json"
CARD = REPO / "data" / "d478_hand_scorecard.json"
OUT = REPO / "data" / "d480_hand_scorecard.json"
CHART = REPO / "temp" / "d480_hand_chart.json"
...
def lines_from_arrays(RC, op, cl, hi, lo, piv, cell, margin, delta, max_dg):
    """The construction on arrays alone (D481's workers): no bar objects, no fixture modules."""
    m = op.size
...
def main() -> int:
    ...
    SC = _load("d451sc", "d478_score_hand_lines.py")
    ...
    for name in ("PIVOT", "GROW", "HYST", "HYST-WIDE"):
        fmt(name, card[name])
    fmt("HAND-CELL", rows)
    OUT.write_text(json.dumps(dict(what="D480: D399's construction re-dialled against the hand-drawn lines",
    ...
    CHART.write_text(json.dumps(dict(
        ...
        page=dict(title="The hand cell against the hand lines",
                  eyebrow="D480 &middot; the pivot construction re-dialled against the principal's hand-drawn lines",
                  cfg=SETTINGS_LINE_HAND,
                  lede1=("D399's pivot construction re-dialled against the principal's hand-drawn lines "
                         "(D480): <b>pair rules off, one-bar pivots, no width kill, one pivot carried across "
