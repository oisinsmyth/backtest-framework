...
THE RULE IS D434's, UNCHANGED -- `entry_mask`, `walk`, `summarise` and `atr_of` are imported from
...
floor. The comparison is per-trade, cell for cell, against `data/d434_channel_trades.json`.
...
def oracle_channels(RC, lo, hi, a0=0, a1=None):
    ...
    similarity gate can then never bind, since 0.4 is tighter than any tau in D434's grid."""
...
def oracle_lines(RC, bars, segs=None):
    ...
    and the GONE exit behave identically to D434."""
    m = len(bars)
