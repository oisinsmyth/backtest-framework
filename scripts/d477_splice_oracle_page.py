"""THE ORACLE PAGE: perfect-knowledge channels, every parameter adjustable, twelve names.
    ...
    uv run python scripts/d477_emit_live_bars.py     (or reuse temp/d399_live_bars.json)
    uv run python scripts/d477_splice_oracle_page.py

A PORT, NOT A CALL, exactly as the causal page is: `envelope_fit` and the greedy maximal-channel
search of `run_d477_oracle_ceiling.oracle_channels` are re-implemented in the page's JavaScript,
...
OUT = (Path(sys.argv[1]).resolve() if len(sys.argv) > 1 else REPO / "temp" / "d477_oracle_page.html")
...
<div class="wrap">
  <div class="eyebrow">D477 &middot; D478 &middot; perfect-knowledge channels, and the same algorithm made causal</div>
  ...
  <p class="lede"><b>Grow right (CAUSAL)</b> is the left-to-right oracle with its two hindsight
    ...
    hindsight. This is D478's candidate to replace D399's pivot construction.</p>
...
(function(){
  ...
  // GROW RIGHT, CAUSALLY (D478): the left-to-right oracle with its two hindsight leaks removed.
