"""D478 -- THE CAUSAL CONSTRUCTION REBUILT FROM THE ORACLE: grow right, one bar at a time.

    uv run python scripts/d478_causal_grow.py --proof        twelve live names, audits, timing

WHY. D477's oracle is greedy left-to-right growth: seed a minimum-length window, extend it while
...
AUDITS.
  ...
  [F]  truncating the series at any bar t and recomputing gives the same lines up to t -- the
       construction reads nothing after the bar. The trader-side [F] in D477 tested the RULE; this
       tests the LINES.
"""
...
# the oracle line it replaced, kept so [O] and the D477 comparison can still be reproduced
...
def fit_window(RC, lo, hi, a, b, P):
    """One window, fitted and filtered: D477's `fit_window` with the constants made parameters.
...
def oracle_onebar(RC, lo, hi, P, a0=0, a1=None):
    """D477's greedy oracle (slack 0) with the stride growth replaced by one bar at a time. The
...
def causal_lines(RC, bars, P, runs=None):
    """The per-bar view the trader is given, shaped exactly like D476's `build_lines` and D477's
