...
def rankdata(x: np.ndarray) -> np.ndarray:
    """Average ranks, ties shared -- scipy.stats.rankdata('average') without scipy."""
    n = len(x)
...
def session_bootstrap_excess(exc: np.ndarray, b0: np.ndarray, sid: np.ndarray,
    ...
    """SE of (mean exception - mean b0) resampling ROOT-SESSIONS, not blocks."""
    ns = int(sid.max()) + 1
    ...
    for i in range(n_boot):
        d = rng.integers(0, ns, ns)
        n = n_s[d].sum()
        out[i] = (e_s[d].sum() - b_s[d].sum()) / n if n > 0 else np.nan
    return out
...
def run() -> int:
    ...
    for H in HORIZONS:
        t0 = time.time()
        zs, excs, b0s, sids, ords, per_root = [], [], [], [], [], {}
        ranks = []
        off = 0
        for r in roots:
            g = d[d["root"] == r]
            ...
            off += n_sess_root
            e, o, ns = split_half(lab["z"], sid, ordn)
            per_root[r] = {
                ...
                "reliability": spearman(e, o) if ns > 30 else None}
        ...
        rel = spearman(e, o)
        # null: replace each block's observed atom by a uniformly drawn one
        null = np.empty(N_NULL)
        RA = np.hstack(ranks) if ranks else None
        for i in range(N_NULL):
            if RA is not None:
                k = rng.integers(0, m, RA.shape[1])
                zn = RA[k, np.arange(RA.shape[1])].astype(np.float64) / (2 * m)
            else:
                zn = rng.random(len(z))       # z is uniform to within 1/m; m >= 32,768 here
            en, on, _ = split_half(zn, sid, ordn)
            null[i] = spearman(en, on)
        # bootstrap SE of the null p95 itself (D373: a sample p95 is biased toward the centre)
        p95 = float(np.percentile(null, 95))
        p95_se = float(np.std([np.percentile(rng.choice(null, len(null)), 95)
                               for _ in range(200)], ddof=1))
        rel_boot = None
        if ns > 30:
            bb = np.empty(500)
            for i in range(500):
                dd = rng.integers(0, ns, ns)
                bb[i] = spearman(e[dd], o[dd])
            rel_boot = float(np.std(bb, ddof=1))

        row = {"H": H, "atoms": m, "exact_null": H <= EXACT_MAX_H,
               "blocks": int(len(z)), "sessions_0a": ns,
               "mean_z": float(z.mean()),
               ...
               "excess_se_pp": float(np.std(bse, ddof=1)) if bse is not None else None,
               "reliability": float(rel), "reliability_se": rel_boot,
               "null_p50": float(np.percentile(null, 50)), "null_p95": p95,
               "null_p95_se": p95_se,
               "per_root": per_root}
        res["ladder"][str(H)] = row
        P(f"\n  H={H:>2}  {len(z):>8,} blocks  {ns:>7,} sessions  {time.time()-t0:5.1f}s"
          f"   mean z {z.mean():.4f}")
        if H >= 6:
            P(f"        0b  exception {exc.mean()*100:6.3f}%  b0 {b0.mean()*100:6.3f}%"
              f"  EXCESS {excess:+7.3f} pp  +/- {np.std(bse, ddof=1):.3f}")
        else:
            P(f"        0b  NOT DEFINED (b0 = 0 exactly; the threshold is eff = 1)")
        P(f"        0a  reliability {rel:+.4f}"
          f"{'' if rel_boot is None else f' +/- {rel_boot:.4f}'}"
    ...
    res["gates"] = {
        ...
        "gate_0a": {"statistic": "split-half Spearman of mean z, odd vs even blocks",
                    "value": rel, "null_p95": p95, "null_p95_se": p95se,
