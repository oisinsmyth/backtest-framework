"""D500 -- hurdle P, all six, on the D495 candidate. P3 has never been computed on it.

    uv run python scripts/d500_candidate_hurdle_p.py --self-test
    uv run python scripts/d500_candidate_hurdle_p.py --run [--json]
...
OUT = REPO / "data" / "d500_candidate_hurdle_p.json"
...
def do_run(as_json: bool) -> int:
    ...
    res = {"generated_utc": datetime.now(timezone.utc).isoformat(timespec="seconds"),
           "purpose": "D500: hurdle P computed on the D495 candidate, P3 for the first time "
