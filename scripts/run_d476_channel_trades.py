"""D476 -- TRADING THE FINAL CHANNEL: enter on agreement, exit on a target or a trailing stop.

    uv run python -u scripts/run_d476_channel_trades.py --proof     assertions on 3 names
    uv run python -u scripts/run_d476_channel_trades.py             the whole panel
...
OUT = REPO / "data" / "d476_channel_trades.json"
...
def main() -> int:
    ...
    np.save(REPO / "temp" / "d476_book_headline.npy", book[key])   # so the book can be re-scored
    try:
    ...
    if not a.proof:
        OUT.write_text(json.dumps(dict(
            what="D476: the final channel traded -- enter on agreement, exit on target/trail",
