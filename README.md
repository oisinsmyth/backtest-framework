...
| | |
|---|---|
| **2,490 tests** | hand-computed golden masters · integration · property · unit |
...
| | |
|---|---|
| **830 decision records** | D1 → D602, over **556** decision numbers — a pre-registration and its result share one number |
| **50 library modules** | across 11 packages, plus 25 in `research/`, which is study code rather than framework |
| **63 documents** | in [`docs/results/`](docs/results/README.md), five of them featured |
| **2,609 tests** | 173 golden · 96 property · 159 integration · 2,181 unit, across 174 files |
| **633 research runners** | in `scripts/`, one-shot by design |
...
```bash
uv sync
uv run pytest -q tests/golden     # 133 ledger-anchored tests
```
