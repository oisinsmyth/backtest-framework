...
| | |
|---|---|
| **825 decision records** | D1 → D590, over **553** decision numbers — a pre-registration and its result share one number |
| **49 library modules** | across 11 packages, plus 25 in `research/`, which is study code rather than framework |
| **63 documents** | in [`docs/results/`](docs/results/README.md), five of them featured |
| **2,490 tests** | 133 golden · 96 property · 159 integration · 2,102 unit, across 171 files |
| **628 research runners** | in `scripts/`, one-shot by design |
...
docs/
  ...
  decisions/              one record per design call, D1 → D590   [index](docs/decisions/README.md)
...
- [`docs/decisions/`](docs/decisions/README.md) — one file per design decision, D1 → D590; the file and number counts are in the generated block at the top of this page. The index is a curated table for D1–D284 and a generated register below it, and a test fails if any record is in neither half.
