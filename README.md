...
| | |
|---|---|
| **836 decision records** | D1 → D613, over **562** decision numbers — a pre-registration and its result share one number |
...
docs/
  ...
  decisions/              one record per design call, D1 → D613   [index](docs/decisions/README.md)
...
- [`docs/decisions/`](docs/decisions/README.md) — one file per design decision, D1 → D613; the file and number counts are in the generated block at the top of this page. The index is a curated table for D1–D284 and a generated register below it, and a test fails if any record is in neither half.
