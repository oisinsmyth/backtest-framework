...
| | |
|---|---|
| **3,007 tests** | hand-computed golden masters · integration · property · unit |
...
| | |
|---|---|
| **838 decision records** | D1 → D614, over **563** decision numbers — a pre-registration and its result share one number |
| **57 library modules** | across 11 packages, plus 25 in `research/`, which is study code rather than framework |
| **64 documents** | in [`docs/results/`](docs/results/README.md), five of them featured |
| **3,007 tests** | 248 golden · 145 property · 159 integration · 2,455 unit, across 189 files |
| **638 research runners** | in `scripts/`, one-shot by design |
...
```bash
uv sync
uv run pytest -q tests/golden     # 248 ledger-anchored tests
```
...
docs/
  ...
  decisions/              one record per design call, D1 → D614   [index](docs/decisions/README.md)
...
- [`docs/decisions/`](docs/decisions/README.md) — one file per design decision, D1 → D614; the file and number counts are in the generated block at the top of this page. The index is a curated table for D1–D284 and a generated register below it, and a test fails if any record is in neither half.
...
- [`docs/RULES.md`](docs/RULES.md) — standing scope/sequencing rules (R1–R17), apply continuously rather than once
