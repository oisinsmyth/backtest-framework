...
| | |
|---|---|
| **4,152 tests** | hand-computed golden masters · integration · property · unit |
...
| | |
|---|---|
| **847 decision records** | D1 → D621, over **573** decision numbers — a pre-registration and its result share one number |
...
| **4,152 tests** | 421 golden · 234 property · 159 integration · 3,338 unit, across 229 files |
| **658 research runners** | in `scripts/`, one-shot by design |
...
```bash
uv sync
uv run pytest -q tests/golden     # 421 ledger-anchored tests
```
...
docs/
  ...
  decisions/              one record per design call, D1 → D621   [index](docs/decisions/README.md)
...
- [`docs/decisions/`](docs/decisions/README.md) — one file per design decision, D1 → D621; the file and number counts are in the generated block at the top of this page. The index is a curated table for D1–D284 and a generated register below it, and a test fails if any record is in neither half.
