...
ANCHORS: list[tuple[str, str, Callable[[], int]]] = [
    ("panels in the manifest", "127", _panels),
...
def test_the_seven_panels_without_a_blob_are_the_ones_named():
    ...
    programme's two macro panels eleven the same day, and D613's ES option volume panel in ET
    clock buckets twelve.
    """
    prose = PAGE.read_text(encoding="utf-8")
    ...
    assert len(without) == 12, (
        f"{len(without)} panels now have no blob id, and the page's prose says twelve. The count "
