...
ANCHORS: list[tuple[str, str, Callable[[], int]]] = [
    ("panels in the manifest", "126", _panels),
...
def test_the_seven_panels_without_a_blob_are_the_ones_named():
    """A count can hold while the identities drift; the page names all seven, so check the set.
    ...
    D589's session calendar eight, D601's two closure-programme panels ten (2026-09-21) and D613's
    ES option volume panel in ET clock buckets eleven.
    """
    prose = PAGE.read_text(encoding="utf-8")
    ...
    assert len(without) == 11, (
        f"{len(without)} panels now have no blob id, and the page's prose says eleven. The count "
