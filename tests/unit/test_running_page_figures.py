...
ANCHORS: list[tuple[str, str, Callable[[], int]]] = [
    ("panels in the manifest", "132", _panels),
...
def test_the_seven_panels_without_a_blob_are_the_ones_named():
    ...
    panel, Robinhood holders and GDELT hourly sample seventeen (the same day).
    """
    prose = PAGE.read_text(encoding="utf-8")
    ...
    assert len(without) == 17, (
        f"{len(without)} panels now have no blob id, and the page's prose says seventeen. The count "
